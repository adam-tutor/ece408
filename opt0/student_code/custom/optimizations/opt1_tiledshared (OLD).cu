...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    int b = blockIdx.y;
    int m = blockIdx.x;
    //int t = threadIdx.x + threadIdx.y * shared_block_width;
    int ty = threadIdx.y;
    int tx = threadIdx.x;
        ...
        for(int i = threadIdx.y; i < shared_block_width; i += TILE_WIDTH){
            for(int j = threadIdx.x; j < shared_block_width; j += TILE_WIDTH){
                if((h_s * S + i) < H && (w_s * S + j) < W){
                    shared_3d(c, i, j) = in_4d(b, c, S * h_s + i, S * w_s + j);
                }
                else{
                    shared_3d(c, i, j) = 0.0f;
                }
            }
        }
    }
    ...
    if(h < H_out && w < W_out){
        ...
        for(int c = 0; c < C; c++){
            for(int p = 0; p < K; p++){
                for(int q = 0; q < K; q++){
                    value += shared_3d(c, ty * S + p, tx * S + q) * mask_4d(m, c, p, q);
                }
            }
        }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
   ...
   cudaMemcpyToSymbol(mask, host_mask, M * C * K * K * sizeof(float), 0, cudaMemcpyDeviceToHost);
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    int W_grid = ceil((1.0 * (W - K)/S + 1)/TILE_WIDTH);
    int H_grid = ceil((1.0 * (H - K)/S + 1)/TILE_WIDTH);
    int Y_grid = H_grid * W_grid;
    const int shared_block_width = (TILE_WIDTH - 1)*S + K;

    dim3 dimBlock(TILE_WIDTH, TILE_WIDTH, 1);
    dim3 dimGrid(M, B, Y_grid);
