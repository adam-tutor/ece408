...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    int WblockRow = (W_out - 1)/TILE_WIDTH + 1;
    ...
    #define tree_3d(i2, i1, i0) Tree[(i2) * (TILE_WIDTH * C) + (i1) * (C) + i0]
    ...
    if(h < H_out && w < W_out){
        ...
        for(int p = 0; p < K; p++){
            if(!(h + p > H)){
                for(int q = 0; q < K; q++){
                    if(!(w + q > W)){
                        value += in_4d(b, tz, h + p, w + q) * mask_4d(m, tz, p, q);
                        //value += input_ptr[(h + p) * W + (w + q)] * mask_ptr[p * K + q];
                    }
                }
            }
            
        }
        tree_3d(ty, tx, tz) = value;

        /*while(stride < C) {//convolution from MP6 + 7
            __syncthreads();
            if((tz % (2 * stride) == 0) && (tz + stride < C)){
                tree_3d(ty, tx, tz) += tree_3d(ty, tz, tz + stride);
            }
            stride = stride * 2;
        }*/
        for(int stride = 1; stride <= blockDim.z; stride *= 2){
            __syncthreads();
            if(((tz % stride) == 0) && (2*tz + stride < blockDim.z)){
                tree_3d(ty, tx, 2*tz) += tree_3d(ty, tz, 2*tz + stride);
            }
        }
        __syncthreads();
        if(tz == 0){
            out_4d(b, m, h, w) = tree_3d(ty, tx, tz);
        }
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    int kernel_Size = C * TILE_WIDTH * TILE_WIDTH;
